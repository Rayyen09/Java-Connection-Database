...
# ===== MENU: UPDATE PROGRESS =====
    ...
    if not df.empty:
        with st.container():
            ...
            if not df_filtered_display.empty and (filter_track_status or filter_buyer_progress):
                st.markdown("---")
                st.markdown(f"**📋 {len(df_filtered_display)} order sesuai filter:**")
                
                # Display filtered orders in table
                display_cols = st.columns([1.2, 1.2, 1.5, 0.8, 1.2])
                display_cols[0].markdown("**Order ID**")
                display_cols[1].markdown("**Buyer**")
                display_cols[2].markdown("**Produk**")
                display_cols[3].markdown("**Status**")
                display_cols[4].markdown("**Tracking Status**")
                
                for idx, row in df_filtered_display.iterrows():
                    display_row_cols = st.columns([1.2, 1.2, 1.5, 0.8, 1.2])
                    
                    # Clickable order ID
                    if display_row_cols[0].button(row['Order ID'], key=f"select_{idx}", use_container_width=True):
                        selected_order = row['Order ID']
                        st.rerun()
                    
                    display_row_cols[1].write(row['Buyer'])
                    display_row_cols[2].write(row['Produk'][:25] + "..." if len(row['Produk']) > 25 else row['Produk'])
                    ...
                    display_row_cols[3].write(f"{status_colors.get(row['Status'], '')} {row['Status']}")
                    ...
                    track_icon, track_color = tracking_colors.get(row['Tracking Status'], ("⚪", "#6B7280"))
                    display_row_cols[4].markdown(f"<span style='color: {track_color};'>{track_icon} {row['Tracking Status']}</span>", unsafe_allow_html=True)
            
            elif filter_track_status or filter_buyer_progress:
                st.warning("⚠️ Tidak ada order yang sesuai dengan filter yang dipilih.")
            ...
            if selected_order:
                ...
                # Tampilkan tracking status saat ini
                current_tracking = get_tracking_status_from_progress(order_data['Progress'], order_data['Status'])
                ...
                track_icon, track_color = tracking_status_colors.get(current_tracking, ("⚪", "#6B7280"))
                
                st.markdown(f"""
                <div style='background-color: {track_color}22; padding: 10px; border-radius: 8px; border-left: 4px solid {track_color}; margin-bottom: 20px;'>
                    <span style='color: {track_color}; font-weight: bold; font-size: 1.1em;'>{track_icon} Current Tracking Status: {current_tracking}</span>
                </div>
                """, unsafe_allow_html=True)
                ...
                with btn_col2:
                    if st.button("💾 SAVE PROGRESS", use_container_width=True, type="primary"):
                        ...
                        if save_data(st.session_state["data_produksi"]):
                            new_tracking = get_tracking_status_from_progress(f"{progress_value}%", status_order)
